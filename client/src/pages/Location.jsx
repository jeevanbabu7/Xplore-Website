...
export const TextRevealTW = () => {
  const address = `Government College of Engineering Kannur \nMangattuparamba \nParassinikadavu P.O.
Kannur, Kerala
Pin : 670563`;

  // Split the address into lines and characters
  const lines = address.split('\n');

  return (
    <div className="text-white font-handjet font-light text-3xl">
          ...
          <h1 className="text-2xl font-bold leading-6">
            {line.split('').map((char, charIndex) => (
              <span
                ...
                key={`${char}-${charIndex}`}
                style={{ animationDelay: `${(lineIndex * 0.5 + charIndex * 0.05)}s` }}
              >
                {char === " " ? "\u00A0" : char}
              </span>
            ))}
